...
export default {
    ...
    methods:{
        ...
        fetch(lineName){
            this.axios.get(`api/routes/routeCrowdedness&line_name=${store.state.selectedRoute}`)
            .then(res=>{
                this.data = res.data;
                this.maxValue = echarts.util.reduce(this.data, function (max, item) {
        ...
        let option = {
            ...
            tooltip: {
                formatter:function (params) {
                    ...
                    return `周${date[params.value[0]]}    ${store.state.selectedRoute}
                    </br>${params.value[1]}:00到${params.value[1]+1}:00的在乘人数为${params.value[2]}`;
                }
            },
            visualMap: {
                type: 'continuous',
